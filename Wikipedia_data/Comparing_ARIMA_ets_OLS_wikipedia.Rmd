...
##Data reshaping
wikipedia_wide <-
  as.data.frame(matrix(wikipedia_data$views_log, nrow = 394, ncol = 913)) #394: length of each series and 913: number of series
##############
...
error_forecast_hierarchy_arima_ets <- function(total_wikipedia) {
  ...
  for (i in 1:k) {
    ...
    forecast_hierarchy_arima[[length(forecast_hierarchy_arima) + 1]] = as.data.frame(exp(fore_hts_arima$bts))
    forecast_hierarchy_ets[[length(forecast_hierarchy_ets) + 1]] = as.data.frame(exp(fore_hts_ets$bts))
  }
...
error_hierarchy_arima <-
  as.data.frame(exp(validation_wikipedia)) - forecast_hierarchy_arima
forecast_hierarchy_ets <-
  do.call("rbind", final_result_hierarchy_arima_ets[[2]])
error_hierarchy_ets <-
  as.data.frame(exp(validation_wikipedia)) - forecast_hierarchy_ets
###
...
validation_set <-
  as.data.frame(matrix(
    wikipedia_data.valid.train[[2]]$views_log,
    nrow = 28,
    ncol = 913
  ))
names(validation_set) <-
  unique(wikipedia_data.valid.train[[1]]$cat_column)
...
List_1 <- split(wikipedia_data$views_log, wikipedia_data$cat_column)
...
names(wikipedia_data_1) <-
  c("cat_column", "views_log", "Trend", "Seasonality", "Split")
category_uni <- (unique(wikipedia_data_1$cat_column))
lag_making <- list()
for (i in seq_along(category_uni))
  lag_making[[i]] <-
  Lag(wikipedia_data_1$views_log[wikipedia_data_1$cat_column == category_uni[i]], 1:7)
...
fore_error_hierarchy_OLS <- function(df) {
  k <- 28
  forecast_hierarchy_OLS<-c()
    for (i in 1:k) {
    n <- nrow(df)
    train.1 <- df[1:((n - k) + (i - 1)), ]
    valid.1 <- df[(n - k) + i, ]
    hierarchy_multiple_OLS_model <-
      lm(views_log ~ Trend + Seasonality + Lag.1 + Lag.2 + Lag.3 + Lag.4 + Lag.5 + Lag.6 + Lag.7, 
         data = train.1)
    fore_hierarchy_multiple_OLS <-
      predict.lm(hierarchy_multiple_OLS_model, newdata = valid.1,type = "response")
    forecast_hierarchy_OLS<-c(forecast_hierarchy_OLS,fore_hierarchy_multiple_OLS)
    }
...
forecast_hierarchy_OLS<-exp(forecast_hierarchy_OLS)
error_hierarchy_OLS<-as.data.frame(exp(validation_set))-as.data.frame(forecast_hierarchy_OLS)
...
forecast_error_arima_ets <- function(df) {
  ...
  for (i in 1:k) {
    ...
    error_arima_ <-
      as.numeric(exp(valid.ts))- as.numeric(exp(pred_arima$mean))
    fit_ets <- ets(train.ts)
    pred_ets <- forecast(fit_ets, h = 1)
    error_ets_ <-
      as.numeric(exp(valid.ts))- as.numeric(exp(pred_ets$mean))
    
    forecast_arima[[length(forecast_arima) + 1]] = as.data.frame(exp(pred_arima$mean))
    error_arima[[length(error_arima) + 1]] = as.data.frame(error_arima_)
    forecast_ets[[length(forecast_ets) + 1]] = as.data.frame(exp(pred_ets$mean))
...
for (i in seq_along(category_uni))
  lag_making[[i]] <-
  Lag(wikipedia_data$Views_log[wikipedia_data$cat_column == category_uni[i]], 1:7)
lag_making <- do.call(rbind, lag_making)
### dataset with lags
...
error_forecast_OLS <- function(df) {
  ...
  for (i in 1:k) {
    ...
    error_OLS_ <-
      as.vector((df.valid$views - exp(fore_OLS)))
    forecast_OLS[[length(forecast_OLS) + 1]] = as.data.frame(exp(fore_OLS))
