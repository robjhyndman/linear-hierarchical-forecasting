...
actual.sim <- readr::read_csv('actual.sim.noise.FH.csv')
...
for(i in 1:k){
  ...
  Sigma.mat <- as(Matrix::diag(fc.OLS.PI.1[i,]) + (Matrix::diag(fc.OLS.PI.1[i,]) %*% H.matrix), 'dgCMatrix')
...
fc.OLS.fix.noise.FH <- bind_rows(fc.OLS.1.48 %>%
...
write_csv(fc.OLS.fix.noise.FH, 'fc.OLS.fix.noise.FH.csv')
