...
TourismData<-TourismData[,-c(1,2)]
k<-12
n<-nrow(TourismData)
names<-names(TourismData)
...
total_tourist <-
  ts(TourismData,
     start = c(1, 1),
     end=c(1, n),frequency = 4)
validation_tourist <-
  window(total_tourist,
         start = c(1, ((n-k)+1)),
         end = c(1, n))
ARIMAETSmodel_hierarchy <- function(total_tourist) {
   forecast_arima <- list()
   forecast_ets <- list()
    for (i in 1:k) {
    ### training set
    ...
    g_tourist_hts <-gts(training_tourist, groups = grouping_hts)
...
final_result_hierarchy <-
  ARIMAETSmodel_hierarchy(total_tourist)
...
h<-12
n<-nrow(TourismData)
validation_set<-TourismData[((n-h)+1):n,]
names <- names(TourismData)
#### group defining
name_length <- str_length(names)
g <- rbind(
...
gts_TourismData <- gts(TourismData, groups = g)
ally_TourismData <- as.data.frame(aggts(gts_TourismData))
List_2 <- do.call(c, apply(ally_TourismData, 2, list))
...
result_fore_base_hierarchy_OLS<-sapply(List_2, OLSmodel,freq=4,h=12)
y.f.recon <-
  combinef(result_fore_base_hierarchy_OLS,
           groups = get_groups(gts_TourismData),
           keep = "gts",
           algorithms = "lu")
...
write.csv(forecast_hierarchy_OLS_recon,"forecast_hierarchy_OLS.csv")
...
write.csv(error_hierarchy_OLS_recon,"error_hierarchy_OLS.csv")
```
...
k<-12
n<-nrow(TourismData)
n.series<-ncol(TourismData)
List <- do.call(c, apply(TourismData, 2, list))
ARIMAETSmodel <- function(df) {
  ...
  for (i in 1:k) {
    ...
    result_fore_arima[[length(result_fore_arima) + 1]] = as.data.frame(fore_arima$mean)
    result_fore_arima[result_fore_arima<0]<-0
    result_error_arima[[length(result_error_arima) + 1]] = as.data.frame(as.numeric(valid.ts) - as.numeric(fore_arima$mean))
    result_fore_ets[[length(result_fore_ets) + 1]] = as.data.frame(fore_ets$mean)
    result_fore_ets[result_fore_ets<0]<-0
    result_error_ets[[length(result_error_ets) + 1]] = as.data.frame(as.numeric(valid.ts) - as.numeric(fore_ets$mean))
  }
  return(list(result_fore_arima, result_error_arima,result_fore_ets,result_error_ets))
}
forecast_error_arima_ets = lapply(List, ARIMAETSmodel) 
##### forecast arima
...
n<-nrow(TourismData)
validation_set<-TourismData[((n-h)+1):n,]
List_1 <- do.call(c, apply(TourismData, 2, list))
OLSmodel<-function(X,freq,h){
  trend<-seq(NROW(X))
  season<-forecast::seasonaldummy(ts(X,frequency = freq))
  Xlag<-quantmod::Lag(X,k=1:freq)
...
result_fore_OLS<-as.data.frame(sapply(List_1, OLSmodel,freq=4,h=12))
### matching two data frames order to calculate the errors
...
RMSE_arima<-RMSE_arima[match(rownames(RMSE_arima), rownames(RMSE_hierarchy_arima)), ]
...
RMSE_ets<-RMSE_ets[match(rownames(RMSE_ets), rownames(RMSE_hierarchy_ets)), ]
...
RMSE_OLS<-RMSE_OLS[match(rownames(RMSE_OLS), rownames(RMSE_hierarchy_OLS)), ]
...
## TourismData: Comparing OLS methods - with and without hierarchy - Forecasted Series Plot
