...
## this function returns fc.se (the forecast standard errors for computing prediction intervals)

olsfc.se <- function(x, h, maxlag = 0, nolag = NULL ) {
  ...
  {
    model.fit <- predict(fit, newdata = newdata, se.fit = TRUE)
    fc[i] <- model.fit$fit
    fc.se[i] <-  model.fit$se.fit
    # Update newdata
    if(maxlag > 0)
    {
